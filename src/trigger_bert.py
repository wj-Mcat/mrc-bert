...
tf.flags.DEFINE_string("exp_type", "ccks_identify_trigger", "")
...
class InputExample(object):
    ...
    def __init__(self, guid, text, event_type, triggers=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask,
                 segment_ids, event_type_mask, label_ids):
...
class IdentifyTriggerProcessor(DataProcessor):
    def _read_data(self, input_file):
        with open(input_file) as f:
            lines = []
            for line in f:
                sent, event_type, triggers = line.strip('\n').split('\t')
                lines.append((sent.replace(' ', '_'), event_type, triggers))
    ...
    def get_labels(self, exp_type=None):
        if exp_type == "ccks_identify_trigger":
            return ['O', 'B', 'I']
        else:
            return []
    ...
    def _create_example(lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            triggers = line[2]
            examples.append(InputExample(guid, text, event_type, triggers))
        return examples
...
def convert_single_example(ex_index, example, label_map, max_seq_length, tokenizer, mode):
    ...
    triggers = example.triggers.split('^')
    for trigger in triggers:
        if trigger == "":
            continue
        word, start_pos, end_pos = trigger.split('|')
    ...
    if len(tokens) >= max_seq_length - 10:
        return None
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 segment_ids, labels, event_type_mask, num_labels, use_one_hot_embeddings):
    ...
    final_input = tf.concat([sent_features, event_type_features], axis=-1)
...
def main(_):
    ...
    processors = {
        "identify_trigger": IdentifyTriggerProcessor,
    }
