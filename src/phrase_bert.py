...
class InputExample(object):
    ...
    def __init__(self, guid, text, triggers=None):
...
class IdentifyTriggerProcessor(DataProcessor):
    def _read_data(self, input_file):
        with open(input_file) as f:
            lines = []
            for line in f:
                sent, triggers = line.strip('\n').split('\t')
                lines.append((sent.replace(' ', '_'), triggers))
    ...
    def _create_example(lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            triggers = line[1]
            examples.append(InputExample(guid, text, triggers))
        return examples
    ...
    if FLAGS.do_predict:
        ...
        with open(output_predict_file, 'w') as writer:
            for prediction in result:
                ...
                output_line = " ".join(id2label[i] for i in prediction if i != 0) + "\n"
